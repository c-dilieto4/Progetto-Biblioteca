...
\begin{titlepage}
    ...
    \begin{center}
    {\Large Documento di Specifica dei Requisiti}
    \end{center}
    ...
    {\Large Versione 1.3 - 13 Dicembre 2025\par}
...
\begin{itemize}
\item Il Bibliotecario seleziona l’opzione “Aggiungi” nella sezione "Catalogo Libri" e inserisce i dati richiesti (ISBN, titolo, anno di pubblicazione, autore e numero di copie totali).
...
\item Il sistema controlla che il numero di copie rispetti il formato [DF-1].
...
\begin{itemize}
\item Il Bibliotecario seleziona l’opzione “Aggiungi” nella sezione "Anagrafica Utenti" e inserisce i dati richiesti (matricola, nome, cognome, email ecc).
...
\begin{itemize}
\item Il Bibliotecario seleziona l’utente cliccando sul pulsante "Modifica", visualizza e modifica i dati (Nome, Cognome, Email, ecc.). 
...
\item Se la matricola viene modificata, il sistema verifica che l'utente non abbia prestiti attivi in corso. Se presenti, l'operazione viene bloccata per garantire l'integrità referenziale ([FC-3]).
...
\begin{itemize}
\item Il Bibliotecario identifica il libro da voler aggiornare (es. tramite ISBN del libro) cliccando sul tasto "Modifica".
...
\item Se l'ISBN viene modificato, il sistema verifica che non esistano prestiti attivi associati a quel libro [FC-3]. Se presenti, la modifica dell'ISBN viene impedita.
...
\begin{itemize}
\item Il Bibliotecario identifica l’utente da eliminare (es. tramite matricola) cliccando sull'apposito pulsante "Elimina".
...
Business Value: Alto\\
Rischio Tecnico: Basso
\subsubsection{[FC-3] Vincolo cancellazione/modifica dati}
L'SGBU non deve permettere la cancellazione o la modifica del codice identificativo (ISBN/Matricola) di un libro o di un utente se esistono prestiti attivi correlati a tali entità.\\
Business Value: Alto\\
Rischio Tecnico: Basso
...
Il sistema deve implementare un meccanismo di autenticazione tramite credenziali ad esso dedicate (es. login con nome utente e password) per garantire che solo il personale autorizzato possa accedere e operare sul sistema. Al fine di consentire il primo accesso e le fasi di test, il sistema viene fornito con un account predefinito avente le seguenti credenziali: Username: 'admin', Password: 'password'.\\
Business Value: Alto\\
Rischio Tecnico: Medio
...
Nota di Riferimento ai Business Flow: I Casi d'Uso descritti in questa sezione rappresentano le interazioni ad alto livello tra il Bibliotecario e il Sistema. Per il dettaglio procedurale e la sequenza logica esatta delle operazioni, si rimanda esplicitamente ai Business Flow [BF] definiti nella sezione 3.2. Ogni passo del "Flusso Normale" di un Caso d'Uso che implica una transazione sui dati deve intendersi come l'esecuzione del corrispondente Business Flow.
...
\item Se l'ISBN viene modificato e sono presenti prestiti attivi per quel libro, l'operazione viene bloccata per garantire l'integrità dei dati ([FC-3]).
\end{itemize}
Post-condizioni:
\begin{itemize}
...
\item Se la matricola viene modificata e l'utente ha prestiti attivi in corso, l'operazione viene bloccata per garantire l'integrità dei dati ([FC-3])
...
\begin{figure}[h]
    \centering
    \includegraphics[width=1.1\textwidth]{Diagramma_CU}
